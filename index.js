...
const UUID = process.env.UUID || 'af6a8f17-aa3d-4fb0-94b3-307b550706af';
const JK_SERVER = process.env.JK_SERVER || 'nezha.xiaopang66.xyz';
const JK_PORT = process.env.JK_PORT || '443';
const JK_KEY = process.env.JK_KEY || 'RXkCgFY81UAzuTdC9p';
const YM_DOMAIN = process.env.YM_DOMAIN || 'glch0615.linuxxx.shop';
const YM_AUTH = process.env.YM_AUTH || 'eyJhIjoiOGI3YTQ4ZTNhNWIwMzc0ZjlkZmVmNDExM2NiMzFjNTkiLCJ0IjoiMDU5ZGQ5ODYtZDgwMS00ZTJiLTkyN2YtMjdlYzA5ZTRhNzg1IiwicyI6Ik5XTTVZelF5T1dJdFlUQTNOQzAwTVRZNExUZzFOakl0WWpVNU5UQmhNV0V5WTJRMyJ9';
const CFIP = process.env.CFIP || 'skk.moe';
const CFPORT = process.env.CFPORT || 443;
const NAME = process.env.NAME || 'Vls';
const ARGO_PORT = process.env.ARGO_PORT || 27070;
...
const config = {
  ...
  inbounds: [
    { port: ARGO_PORT, protocol: 'vless', settings: { clients: [{ id: UUID, flow: 'xtls-rprx-vision' }], decryption: 'none', fallbacks: [{ dest: 8001 }, { path: "/vless512", dest: 8002 }, { path: "/vmess258", dest: 8003 }, { path: "/trojan258", dest: 8004 }] }, streamSettings: { network: 'tcp' } },
    { port: 8001, listen: "127.0.0.1", protocol: "vless", settings: { clients: [{ id: UUID }], decryption: "none" }, streamSettings: { network: "ws", security: "none" } },
    { port: 8002, listen: "127.0.0.1", protocol: "vless", settings: { clients: [{ id: UUID, level: 0 }], decryption: "none" }, streamSettings: { network: "ws", security: "none", wsSettings: { path: "/vless258" } }, sniffing: { disable: false, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
    { port: 8003, listen: "127.0.0.1", protocol: "vmess", settings: { clients: [{ id: UUID, alterId: 0 }] }, streamSettings: { network: "ws", wsSettings: { path: "/vmess512" } }, sniffing: { disable: false, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
    { port: 8004, listen: "127.0.0.1", protocol: "trojan", settings: { clients: [{ password: UUID }] }, streamSettings: { network: "ws", security: "none", wsSettings: { path: "/trojan512" } }, sniffing: { disable: false, destOverride: ["http", "tls", "quic"], metadataOnly: false } },
  ],
